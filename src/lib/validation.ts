...
// ----------------------
// Update Type: Added
// Description: Created new Validation Library for form validation
// Updated By: Himanshu
...
export class Validator {
  ...
  sanitizeInput(field: string, value: string): string {
    ...
    if (field === 'content') {
      return sanitized.substring(0, 500000); // 500KB limit for blog content
    } else if (field === 'excerpt') {
      return sanitized.substring(0, 5000); // 5KB for excerpt
    } else {
      return sanitized.trim().substring(0, 2000); // 2KB default limit
    }
  }
