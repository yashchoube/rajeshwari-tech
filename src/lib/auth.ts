...
// ----------------------
// Update Type: Changed
// Description: Updated Auth Library to use database sessions
// Updated By: Himanshu
...
// Sessions persist across server restarts and serverless function invocations
...
export class AuthService {
  ...
  static createSession(user: AdminUser): string {
    ...
    createSession(sessionId, user, expiresAt);
  ...
  static getSession(sessionId: string): AdminUser | null {
    try {
      ...
      if (!session) {
        console.log('AuthService.getSession - session not found in DB');
        return null;
      }
