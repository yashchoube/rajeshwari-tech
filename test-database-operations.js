...
async function testBlogOperations() {
  ...
  const blogData = {
    ...
    content:
      "<p>This is test blog content with <strong>HTML formatting</strong> and <em>various elements</em>.</p>",
    author: "Test Author",
    category: "Technology",
  ...
  if (createResult.success) {
    ...
    if (createResult.data.blogId) {
      ...
      if (approveResult.success) {
        ...
        addTestResult(
          "Blog Approval",
          "error",
          `Blog approval failed: ${approveResult.error}`,
        );
      }
    }
  } else {
    addTestResult(
      "Blog Creation",
      "error",
      `Blog creation failed: ${createResult.error}`,
    );
  }
  ...
  if (blogsResult.success) {
    addTestResult(
      "Blog Retrieval",
      "success",
      `Retrieved ${blogsResult.data.blogs?.length || 0} blogs`,
    );
  } else {
    addTestResult(
      "Blog Retrieval",
      "error",
      `Blog retrieval failed: ${blogsResult.error}`,
    );
  }

  // Test admin blog retrieval
  const adminBlogsResult = await testAPI(
    "/api/blogs?scope=admin",
    "GET",
    null,
    200,
  );
  if (adminBlogsResult.success) {
    addTestResult(
      "Admin Blog Retrieval",
      "success",
      `Retrieved ${adminBlogsResult.data.blogs?.length || 0} admin blogs`,
    );
  } else {
    addTestResult(
      "Admin Blog Retrieval",
      "error",
      `Admin blog retrieval failed: ${adminBlogsResult.error}`,
    );
  }
}
...
async function testEnrollmentOperations() {
  ...
  const enrollmentResult = await testAPI(
    "/api/enrollment",
    "POST",
    enrollmentData,
    200,
  ...
  if (enrollmentResult.success) {
    ...
    addTestResult(
      "Course Enrollment",
      "error",
      `Course enrollment failed: ${enrollmentResult.error}`,
    );
  }
}
...
async function testEnquiryOperations() {
  ...
  if (enquiryResult.success) {
    ...
    addTestResult(
      "Enquiry Submission",
      "error",
      `Enquiry submission failed: ${enquiryResult.error}`,
    );
  }
}
...
async function testAnalyticsOperations() {
  ...
  if (analyticsResult.success) {
    ...
    addTestResult(
      "Analytics Tracking",
      "error",
      `Analytics tracking failed: ${analyticsResult.error}`,
    );
  }
}
...
async function testNewsletterOperations() {
  ...
  const subscriptionResult = await testAPI(
  ...
  if (subscriptionResult.success) {
    ...
    addTestResult(
      "Newsletter Subscription",
      "error",
      `Newsletter subscription failed: ${subscriptionResult.error}`,
    );
  }

  // Test newsletter subscribers retrieval
  const subscribersResult = await testAPI(
  ...
  if (subscribersResult.success) {
    addTestResult(
      "Newsletter Subscribers Retrieval",
      "success",
      `Retrieved ${subscribersResult.data.subscriptions?.length || 0} newsletter subscriptions`,
    );
  } else {
    addTestResult(
      "Newsletter Subscribers Retrieval",
      "error",
      `Newsletter subscribers retrieval failed: ${subscribersResult.error}`,
    );
  }
}
...
async function testErrorScenarios() {
  ...
  if (!blogResult.success) {
    addTestResult(
      "Invalid Blog Data Handling",
      "success",
      "Correctly rejected invalid blog data",
    );
  } else {
    addTestResult(
      "Invalid Blog Data Handling",
      "error",
      "Should have rejected invalid blog data",
    );
  }
  ...
  if (!enrollmentResult.success) {
    addTestResult(
      "Invalid Enrollment Data Handling",
      "success",
      "Correctly rejected invalid enrollment data",
    );
  } else {
    addTestResult(
      "Invalid Enrollment Data Handling",
      "error",
      "Should have rejected invalid enrollment data",
    );
  }

  // Test non-existent endpoints
  const notFoundResult = await testAPI(
  ...
  if (!notFoundResult.success) {
    ...
    addTestResult(
      "404 Error Handling",
      "error",
      "Should have returned 404 for non-existent endpoint",
    );
  }
}
...
function generateTestReport() {
    ...
    summary: {
      ...
      successRate:
        TEST_RESULTS.total > 0
          ? ((TEST_RESULTS.passed / TEST_RESULTS.total) * 100).toFixed(2)
          : 0,
    },
  ...
  log(
    `Test Summary: ${TEST_RESULTS.passed}/${TEST_RESULTS.total} passed (${report.summary.successRate}%)`,
  );
